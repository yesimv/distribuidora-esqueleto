...
document.addEventListener("DOMContentLoaded",async function () {
    ...
    //busca la tabla de analisis y si la encuentra, la genera
    if (document.querySelector('#tabla-analisis')) {
        await  tablaAnalisis();
    }
    

    fp('#fch-fin-analisis', configFecha);
    fp('#fch-inicio-analisis', configFecha);
...
const tablaAnalisis = async (fechas = null) => {
    const columnas = [
        { data: 'id_analisis_tecnico', title: 'ID' },
        {
...
const filtrarPorFecha = async () => {
    const fInicio = document.getElementById('fch-inicio-analisis').value;
    const fFinal = document.getElementById('fch-fin-analisis').value;
    const fechas = {
        'fchInicio': fInicio,
        'fchFinal': fFinal
    };
